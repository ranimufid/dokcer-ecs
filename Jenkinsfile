  ...
  stages {
    ...
    stage ('terraform plan'){
      steps {
        ...
        sh 'env'
        script {
          env.TF_LANDSCAPE_PLAN= sh(returnStdout: true, script: 'terraform plan -out $(echo $GIT_COMMIT | cut -c1-7)-$(git show -s --pretty=%an).plan -input=false -detailed-exitcode | landscape')
