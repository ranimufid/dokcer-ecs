pipeline {
  ...
  stages {
    ...
    stage ('slack'){
      steps {
        slackSend (color: 'good', message: "Plan Awaiting Approval: ${env.JOB_NAME} - ${env.BUILD_NUMBER} ```hello snookums ${env.TF_LANDSCAPE_PLAN}```", teamDomain: "${env.SLACK_TEAM_DOMAIN}", token: "${env.SLACK_TOKEN}")
        script {
          try {
